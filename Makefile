...
.PHONY: ord-extract-lat
ord-extract-lat:
	@$(call _mkstdirs)
	@echo "[ORD] all-sink delivered latency extraction (no 5000-path cap)"
	$(call _run_with_tmp_log,$(LOG_DIR)/4_9_extract_lat.log,SC_FILE="$(SC_LEF_CTS)" SC_LEF="$(SC_LEF_CTS)" LEF_FILES="$(LEF_FILES_CTS)" ADDITIONAL_LEFS="$(ADDITIONAL_LEFS_CTS)" COVER_LAYER="$(COVER_LAYER)" $(TIME_CMD) $(OPENROAD_CMD) $(OPENROAD_SCRIPTS_DIR)/extract_all_sink_latency.tcl)
...
ord-open:
	@$(call _mkstdirs)
	@echo "[ORD] opening pre-built layout in the GUI ..."
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/open_layout.tcl,$(LOG_DIR)/open.log)
...
.PHONY: ord-extract-final
ord-extract-final:
	@$(call _mkstdirs)
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/extract_final_graph.tcl,$(LOG_DIR)/extract_final.log)

.PHONY: ord-eval-lp
ord-eval-lp:
	@$(call _mkstdirs)
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/eval_lp_solution.tcl,$(LOG_DIR)/eval_lp.log)

.PHONY: ord-exhaust
ord-exhaust:
	@$(call _mkstdirs)
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/exhaust_extract.tcl,$(LOG_DIR)/exhaust.log)

.PHONY: ord-eval-split
ord-eval-split:
	@$(call _mkstdirs)
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/eval_lp_split.tcl,$(LOG_DIR)/eval_split.log)

.PHONY: ord-audit
ord-audit:
	@$(call _mkstdirs)
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/completeness_audit.tcl,$(LOG_DIR)/audit.log)

.PHONY: ord-probe
ord-probe:
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/probe_ep.tcl,$(LOG_DIR)/probe.log)

.PHONY: ord-perturb
ord-perturb:
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/perturb_one.tcl,$(LOG_DIR)/perturb.log)

.PHONY: ord-unit
ord-unit:
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/unit_probe.tcl,$(LOG_DIR)/unit.log)

.PHONY: ord-oracle
ord-oracle:
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/oracle_capture.tcl,$(LOG_DIR)/oracle.log)

.PHONY: ord-evdel
ord-evdel:
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/eval_delivered.tcl,$(LOG_DIR)/evdel.log)

.PHONY: ord-dumpslack
ord-dumpslack:
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/dump_slack.tcl,$(LOG_DIR)/dumpslack.log)

.PHONY: ord-evalvec
ord-evalvec:
	@$(call _mkstdirs)
	$(call _run_with_tmp_log,$(LOG_DIR)/evalvec.log,SC_FILE="$(SC_LEF_CTS)" SC_LEF="$(SC_LEF_CTS)" LEF_FILES="$(LEF_FILES_CTS)" ADDITIONAL_LEFS="$(ADDITIONAL_LEFS_CTS)" COVER_LAYER="$(COVER_LAYER)" $(TIME_CMD) $(OPENROAD_CMD) $(OPENROAD_SCRIPTS_DIR)/eval_vec.tcl)

# EXPERIMENT: sweep the C++ LP over a set of
# pre-filtered FF-graphs (one per window W) in one process. EXP_DIR/WLIST passed in.
.PHONY: ord-lpsweep
ord-lpsweep:
	@$(call _mkstdirs)
	$(call _run_with_tmp_log,$(LOG_DIR)/lpsweep.log,EXP_DIR="$(EXP_DIR)" WLIST="$(WLIST)" SC_FILE="$(SC_LEF_CTS)" SC_LEF="$(SC_LEF_CTS)" LEF_FILES="$(LEF_FILES_CTS)" ADDITIONAL_LEFS="$(ADDITIONAL_LEFS_CTS)" COVER_LAYER="$(COVER_LAYER)" $(TIME_CMD) $(OPENROAD_CMD) $(OPENROAD_SCRIPTS_DIR)/lp_sweep.tcl)

# EXPERIMENT: full-STA ideal-clock eval of L_bal + a_i(W) for each W, in one process.
.PHONY: ord-evalsweep
ord-evalsweep:
	@$(call _mkstdirs)
	$(call _run_with_tmp_log,$(LOG_DIR)/evalsweep.log,EXP_DIR="$(EXP_DIR)" WLIST="$(WLIST)" SC_FILE="$(SC_LEF_CTS)" SC_LEF="$(SC_LEF_CTS)" LEF_FILES="$(LEF_FILES_CTS)" ADDITIONAL_LEFS="$(ADDITIONAL_LEFS_CTS)" COVER_LAYER="$(COVER_LAYER)" $(TIME_CMD) $(OPENROAD_CMD) $(OPENROAD_SCRIPTS_DIR)/eval_sweep.tcl)

.PHONY: ord-reporthold
ord-reporthold:
	@$(call _mkstdirs)
	$(call _run_with_tmp_log,$(LOG_DIR)/reporthold.log,SC_FILE="$(SC_LEF_CTS)" SC_LEF="$(SC_LEF_CTS)" LEF_FILES="$(LEF_FILES_CTS)" ADDITIONAL_LEFS="$(ADDITIONAL_LEFS_CTS)" COVER_LAYER="$(COVER_LAYER)" $(TIME_CMD) $(OPENROAD_CMD) $(OPENROAD_SCRIPTS_DIR)/report_th.tcl)

.PHONY: ord-postcts
ord-postcts:
	@$(call _mkstdirs)
	$(call _or,$(OPENROAD_SCRIPTS_DIR)/postcts_check.tcl,$(LOG_DIR)/postcts.log)
