...
# iteration=1 routing standard (openroad/innovus common): a few unrouted nets are
# tolerated so the routed DEF is still written for the post-route timing comparison
# (matches Pin3D baseline routing effort). Changed error->warn.
if { ![design_is_routed] } {
  puts "\[WARNING\] Design has unrouted nets (iter=1) — writing routed DEF anyway for timing."
}
