#!/bin/bash
# Full F2F 3D flow for asap7_nangate45_3D/ariane133:
...
# Macro-dominated design: keep the latency balancer off so cross-tier
# macro-to-register hold paths are not degraded (see README, macro handling).
export CTS_NO_INSERTION_DELAY=1
...
# Macro-containing design: place macros between the 3D floorplan and PDN so the
# power grid and the later placement steps see their final locations.
...
# 4) Route with Innovus (Pin-3D environment), then final report in OpenROAD
